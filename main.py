    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                continue
            raise ConnectionError("Blocked by YouTube (IpBlocked/RequestBlocked).")
...
def hf_summarize(text: str) -> str:
    if not ENABLE_SUMMARY:
        return ""
    if not HF_TOKEN:
        return ""  # token not set -> no summary
    ...
    # HF can return 503 while model loads
...
def summarize(req: Req):
    try:
        ...
        summary = hf_summarize(transcript) if req.summarize else ""
        return {"video_id": video_id, "transcript": transcript, "summary": summary}
    except Exception as e:
        return {"error": str(e), "transcript": "", "summary": ""}  # always JSON
